...
fn handle_network_change(app: &AppHandle, old_gateway: Option<String>) {
    ...
    if let Err(e) = crate::commands::sync_windows_proxy_inner(app, &state) {
        log::warn!("Failed to sync PAC after network change: {}", e);
    }
...
fn handle_proxy_change(app: &AppHandle) {
    ...
    log::info!("Windows proxy changed, syncing RuBypass PAC");
    if let Err(e) = crate::commands::sync_windows_proxy_inner(app, &state) {
        log::warn!("Failed to sync PAC after Windows proxy change: {}", e);
    }
